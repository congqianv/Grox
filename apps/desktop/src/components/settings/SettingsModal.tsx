...
function ProviderAndModels() {
  ...
  return <div className="mt-7" data-testid="provider-manager">
    <div className="mb-3 flex items-end justify-between"><div><h3 className="text-[12px] font-medium text-fg">{zh ? "模型服务" : "Model provider"}</h3><p className="mt-1 text-[10px] text-dim">{zh ? "保存或切换供应商后会自动写好模型路由并重连 Agent；常驻模型会直接走该供应商的 Base URL，无需再改 config.toml。密钥不会回传到 WebView。" : "Saving or switching a provider writes model routes and reconnects the agent. Resident models use that provider’s base URL automatically — no config.toml edits. Secrets never return to the WebView."}</p></div><span className="chip">{provider.kind.toUpperCase()}</span></div>
